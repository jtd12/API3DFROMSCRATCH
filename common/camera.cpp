...
{
	camera=new Camera(vector3d(0, 0, 0), vector3d(0, 0, 0), vector3d(0, -1, 0));
	sensitivity=0.1f;
	speed=.5f;
}
...
Matrix4x4 camerasetup::getProjectionMatrix() const {
    return camera->getProjectionMatrix();
}

Matrix4x4 camerasetup::getViewMatrix() const {
    return camera->getViewMatrix(*camera,1);
}
