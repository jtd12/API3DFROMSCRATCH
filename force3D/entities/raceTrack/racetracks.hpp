#ifndef DEF_RACETRACKS_HPP
#define DEF_RACETRACKS_HPP
#include<stdio.h>
#include<iostream>
#include <fstream>
#include <sstream>
...
class raceTrackTrack
{
    ...
    float terrainOffset);
		...
		vector3d closestPointOnSegment(const vector3d& p, const vector3d& a, const vector3d& b);
		vector3d getClosestPointOnTrack(const vector3d& pos, const std::vector<std::pair<vector3d, vector3d>>& trackEdges);
		bool isInsideTrack(const vector3d& pos, const std::vector<std::pair<vector3d, vector3d>>& trackEdges);
		bool pointInTriangle(const vector3d& p, const vector3d& a, const vector3d& b, const vector3d& c);
		bool pointInQuad(const vector3d& p, const vector3d& a, const vector3d& b, const vector3d& c, const vector3d& d);
		vector3d getClosestWaypoint(const vector3d& carPos, const std::vector<vector3d>& waypoints);
